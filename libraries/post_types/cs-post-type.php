 ...
 *
 *     cs_post_type( 'person', array(), array(
 *         'plural' => 'People',
 *         'slug'   => 'meet-the-team'
 *     ) );
 *
 ...
 * @return Extended_CPT
 */
function cs_post_type( $post_type, array $args = array(), array $names = array() ) {
...
class Extended_CPT {
	...
	public function register_post_type() {
		...
		if ( empty( $existing ) ) {
			...
			# This allows us to call `cs_post_type()` on an existing post type to add custom functionality
			# to the post type.
	 ...
	 *
	 * Accepts the same parameters as `cs_taxonomy()`, minus the `$object_type` parameter. Will fall back
	 * to `register_taxonomy()` if Extended Taxonomies isn't present.
	 *
	 * Example usage:
	 *
	 *     $events   = cs_post_type( 'event' );
	...
	public function add_taxonomy( $taxonomy, array $args = array(), array $names = array() ) {
		...
		} else if ( function_exists( 'cs_taxonomy' ) ) {
			cs_taxonomy( $taxonomy, $this->post_type, $args, $names );
		} else {
