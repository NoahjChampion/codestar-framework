...
class Extended_CPT_Test_Setup extends Extended_CPT_Test {
	...
	public function testTaxonomyQueryVarClashTriggersError() {
		...
		try {
			cs_post_type( 'public_taxonomy' );
			$this->fail( 'cs_post_type() should trigger an error when registering a post type which clashes with a taxonomy' );
	...
	public function testInvalidPostTypeTriggersError() {
		...
		try {
			cs_post_type( $name );
			$this->fail( 'cs_post_type() should trigger an error when registering a post type which causes an error' );
		} catch ( PHPUnit_Framework_Error $e ) {
