...
abstract class Extended_CPT_Test extends WP_UnitTestCase {
	...
	protected function register_post_types() {
		...
		$this->cpts['hello']  = cs_post_type( 'hello', $this->args['hello'] );
		$this->cpts['hello']->add_taxonomy( 'hello_category' );

		$this->cpts['person'] = cs_post_type( 'person', array(
			'has_archive'    => 'team',
			'show_in_feed'   => true,
			'site_sortables' => array(
		...
		$this->cpts['nice-thing'] = cs_post_type( 'nice-thing', array(), array(
			'slug' => 'Things',
		) );
		$this->cpts['foo'] = cs_post_type( 'foo', array(
			'rewrite' => array(
		...
		$this->cpts['bar'] = cs_post_type( 'bar', array(
			'public'         => false,
			'featured_image' => 'Icon',
		), array(
			'plural'   => 'Plural',
		...
		$this->cpts['baz'] = cs_post_type( 'baz', array(
			'rewrite' => array(
		...
		$this->cpts['post'] = cs_post_type( 'post', array(
			'labels' => array(
